...
// unpack inputs
    ...
    // reciprocal-multiply constants for get_weight()'s normalization step —
    // see tb/gen_recip_lut.py for the derivation of these exact values.
    // Runtime division by a variable (exp_sum) synthesizes to a deep
    // combinational array divider that crashes OpenLane's ABC step; this
    // replaces it with a table lookup (get_recip) + one multiply, proven
    // bit-exact against true floor division across the full achievable
    // domain by tb/check_recip_lut.py.
...
// Stage 3 — exp() LUT lookup
    ...
    // get_recip(sum) = ceil(2**RECIP_SHIFT / sum), body generated by
    // tb/gen_recip_lut.py — rerun that script if RECIP_SHIFT changes.
    // default: sum is only ever in [255, 2040] by construction (see
    // Stage 4 comment); this branch should never be hit in practice.
...
function automatic logic [OUTPUT_WIDTH-1:0] get_weight(
    ...
    // reciprocal-multiply replaces the division: raw = numerator/sum, but
    // computed as (numerator * get_recip(sum)) >> RECIP_SHIFT — see the
    // RECIP_SHIFT/RECIP_WIDTH comment near LUT_SHIFT for why.
    logic [RECIP_WIDTH-1:0] recip;
    logic [15+RECIP_WIDTH:0] product;  // 36b: 16b numerator x 20b recip
    logic [8:0] raw;                    // post-shift result, max 256 (needs clip below)
    recip = get_recip(sum);
    product = (exp_val_i * 256) * recip;
    raw = product[15+RECIP_WIDTH:RECIP_SHIFT];
    if(raw > 255) begin
        get_weight = 8'd255;
    end else begin
        get_weight = raw[7:0];
